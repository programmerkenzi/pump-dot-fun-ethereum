...
contract TokenFactoryTest is Test {
    ...
    function test_CalculateBuyPrice() public {
        ...
        uint256 remainingTokens = factory.MAX_SUPPLY() - factory.INITIAL_MINT();

        // Calculate the buy price for the remaining tokens
        uint256 buyPrice = factory.calculateBuyPrice(tokenAddress, remainingTokens);

        // Assert that the calculated price equals the total collateral
        assertEq(buyPrice, factory.FUNDING_GOAL(), "Buy price for remaining tokens does not match total collateral");
    }
}
